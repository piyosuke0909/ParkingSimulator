...
public class ParkingLotManager : MonoBehaviour
{
    ...
    private void Start()
    {
        if (logOnStart)
        {
            Debug.Log($"ParkingSlot登録完了: {allSlots.Count}件");
    ...
    public void RegisterAllSlots()
    {
        ...
        foreach (ParkingSlot slot in slots)
        {
            ...
            {
                Debug.LogWarning($"Slot IDが未設定です: {slot.name}");
                continue;
            }

            if (slotDictionary.ContainsKey(slot.slotId))
            {
                Debug.LogWarning($"Slot IDが重複しています: {slot.slotId}");
                continue;
            }
    ...
    public ParkingSlot GetSlotById(string slotId)
    {
        ...
        Debug.LogWarning($"指定されたSlot IDが見つかりません: {slotId}");
        return null;
    }
        ...
        ScenarioRandomService randomService)
    {
        ...
        // FindObjectsの返却順に依存せず、同一seedで同じ候補順になるよう固定します。
